...
class ECGDataset(Dataset):
    def __init__(self, path_to_hdf5, path_to_csv, hdf5_dset="tracings", start_idx=0, end_idx=None):
        ...
        self.labels = pd.read_csv(path_to_csv).values if path_to_csv is not None else None
...
def get_train_and_val_loaders(path_to_hdf5, path_to_csv, hdf5_dset="tracings", batch_size=8, val_split=0.02):
    ...
    train_dataset = ECGDataset(path_to_hdf5, path_to_csv, hdf5_dset, end_idx=n_train)
    valid_dataset = ECGDataset(path_to_hdf5, path_to_csv, hdf5_dset, start_idx=n_train)
